...
const regex = /const { cookies } = await import\("next\/headers"\);[\s\S]*?const sRes = await query\("SELECT \* FROM sites ORDER BY created_at ASC"\);\s*sites = sRes\.rows;\s*\} catch \(err\) \{\}/;
...
fs.writeFileSync('c:/Toon/pssgo/app/topology/page.tsx', c);
console.log('Fixed topology');
