import { getVehicleBrands, toggleVehicleBrandStatus, deleteVehicleBrand } from "@/app/actions/vehicle-brands";
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-3xl font-bold tracking-tight">ยี่ห้อรถยนต์</h1>
              <span className="px-3 py-1 bg-indigo-100 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 text-xs font-bold rounded-full uppercase tracking-wider">Brands</span>
            </div>
            <p className="text-zinc-500 dark:text-zinc-400">
              กำหนดหมวดยี่ห้อรถยนต์ เพื่อเป็นตัวเลือกเวลาเพิ่มข้อมูลรถในระบบ
            </p>
          </div>
          <Link href="/vehicles/vehicle-brands/add" className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-md shadow-indigo-600/20 hover:shadow-lg active:scale-[0.98] flex items-center justify-center gap-2 flex-shrink-0">
        ...
        <div className="bg-white dark:bg-[#121212] rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800/80 overflow-hidden ring-1 ring-zinc-900/5 dark:ring-white/5">
            <div className="overflow-x-auto">
                ...
                <thead>
                    ...
                    <th className="px-6 py-5 text-left text-xs font-semibold text-zinc-500 uppercase tracking-widest w-1/3">ยี่ห้อ (Code/Name)</th>
                ...
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    ...
                    <tr>
                        <td colSpan={4} className="px-6 py-24 text-center">
                            ...
                            <p className="text-zinc-600 dark:text-zinc-300 font-semibold text-lg">ยังไม่มีการกำหนดยี่ห้อรถ</p>
                        </div>
                        </td>
                    </tr>
                    ) : (
                    vehicleBrands.map((brand: any) => (
                        <tr key={brand.id} className="hover:bg-zinc-50 dark:hover:bg-[#1A1A1A] transition-all group">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-mono text-xs">{brand.name.substring(0, 2).toUpperCase()}</span>
                                </div>
                                <div>
                                <div className="text-sm font-bold text-zinc-900 dark:text-white line-clamp-1 font-mono">
                                    {brand.code && <span className="text-indigo-600 dark:text-indigo-500 mr-2">[{brand.code}]</span>}
                                    {brand.name}
                                </div>
                                </div>
                            </div>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 line-clamp-1">{brand.description || '—'}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <form action={async () => {
                                "use server";
                                await toggleVehicleBrandStatus(brand.id, !brand.is_active);
                                }}>
                                <button type="submit" className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border transition-all hover:opacity-80 ${brand.is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' : 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20'}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${brand.is_active ? 'bg-emerald-500' : 'bg-rose-500'}`}></span>
                                    {brand.is_active ? 'ใช้งาน' : 'ระงับ'}
                                </button>
                                </form>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-right">
                            <div className="flex justify-end gap-1">
                                <Link href={`/vehicles/vehicle-brands/${brand.id}`} title="แก้ไขยี่ห้อ" className="text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-500 transition-colors p-2 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-500/10 inline-flex">
