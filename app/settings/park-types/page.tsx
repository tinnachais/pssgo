import { getParkTypes, toggleParkTypeStatus, deleteParkType } from "@/app/actions/park-types";
import Link from "next/link";

export const dynamic = "force-dynamic";

export default async function ParkTypesPage() {
  let parkTypes: any[] = [];
  
  try {
    parkTypes = await getParkTypes();
  } catch (err) {
    console.error("Failed to fetch park types:", err);
  }
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-3xl font-bold tracking-tight">ประเภทการจอดรถ</h1>
                ...
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    ...
                    <tr>
                        <td colSpan={4} className="px-6 py-24 text-center">
                            ...
                            <p className="text-zinc-600 dark:text-zinc-300 font-semibold text-lg">ยังไม่มีการกำหนดประเภทการจอด</p>
                        </div>
                        </td>
                    </tr>
                    ) : (
                    parkTypes.map((type: any) => (
                        <tr key={type.id} className="hover:bg-zinc-50 dark:hover:bg-[#1A1A1A] transition-all group">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <form action={async () => {
                                "use server";
                                await toggleParkTypeStatus(type.id, !type.is_active);
                                }}>
                            ...
                            <div className="flex justify-end gap-1">
                                <Link href={`/settings/park-types/${type.id}`} title="แก้ไขประเภท" className="text-zinc-400 hover:text-amber-600 dark:hover:text-amber-500 transition-colors p-2 rounded-xl hover:bg-amber-50 dark:hover:bg-amber-500/10 inline-flex">
