import { getRevenueType, updateRevenueType } from "@/app/actions/revenue-types";
import { getRevenueGroups } from "@/app/actions/revenue-groups";
import Link from "next/link";
import { notFound } from "next/navigation";

export default async function RevenueTypeDetailsPage(props: { params: Promise<{ id: string }> }) {
  ...
  const type = await getRevenueType(id);
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="bg-white dark:bg-[#121212] rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800/80 p-8 md:p-10 ring-1 ring-zinc-900/5 dark:ring-white/5 relative overflow-hidden group">
            <form action={async (formData) => {
                "use server";
                await updateRevenueType(id, formData);
                ...
                <div className="space-y-2">
                    ...
                    name="group_id"
                    id="group_id"
                    defaultValue={type.group_id?.toString() || ""}
