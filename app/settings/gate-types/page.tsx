import { getGateTypes, toggleGateTypeStatus, deleteGateType } from "@/app/actions/gate-types";
import Link from "next/link";

export const dynamic = "force-dynamic";

export default async function GateTypesPage() {
  let gateTypes: any[] = [];
  
  try {
    gateTypes = await getGateTypes();
  } catch (err) {
    console.error("Failed to fetch gate types:", err);
  }
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-3xl font-bold tracking-tight">ประเภทจุดบริการ</h1>
              <span className="px-3 py-1 bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-xs font-bold rounded-full uppercase tracking-wider">Gate Types</span>
            </div>
            <p className="text-zinc-500 dark:text-zinc-400">
              จัดการประเภทจุดบริการ เช่น ทางเข้า, ทางออก, จุดชำระเงิน
            </p>
          </div>
          <Link href="/settings/gate-types/add" className="bg-amber-600 hover:bg-amber-700 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-md shadow-amber-600/20 hover:shadow-lg active:scale-[0.98] flex items-center justify-center gap-2 flex-shrink-0">
                ...
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    ...
                    <tr>
                        <td colSpan={4} className="px-6 py-24 text-center">
                            ...
                            <p className="text-zinc-600 dark:text-zinc-300 font-semibold text-lg">ยังไม่มีการกำหนดประเภท</p>
                        </div>
                        </td>
                    </tr>
                    ) : (
                    gateTypes.map((type: any) => (
                        <tr key={type.id} className="hover:bg-zinc-50 dark:hover:bg-[#1A1A1A] transition-all group">
                            <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center border border-amber-200 dark:border-amber-500/20 shadow-sm font-bold flex-shrink-0">
                                ...
                                <div className="text-sm font-bold text-zinc-900 dark:text-white line-clamp-1 font-mono">
                                    {type.code && <span className="text-amber-600 dark:text-amber-500 mr-2">[{type.code}]</span>}
                                    {type.name}
                                </div>
                                </div>
                            </div>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <form action={async () => {
                                "use server";
                                await toggleGateTypeStatus(type.id, !type.is_active);
                                }}>
                            ...
                            <div className="flex justify-end gap-1">
                                <Link href={`/settings/gate-types/${type.id}`} title="แก้ไขประเภท" className="text-zinc-400 hover:text-amber-600 dark:hover:text-amber-500 transition-colors p-2 rounded-xl hover:bg-amber-50 dark:hover:bg-amber-500/10 inline-flex">
