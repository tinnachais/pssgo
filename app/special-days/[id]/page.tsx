import { getSpecialDay, updateSpecialDay } from "@/app/actions/special-days";
import Link from "next/link";
import { notFound } from "next/navigation";

export default async function SpecialDayDetailsPage(props: { params: Promise<{ id: string }> }) {
  ...
  const specialDay = await getSpecialDay(id);
    ...
    <div className="min-h-full font-sans selection:bg-amber-500/30">
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold tracking-tight">แก้ไขข้อมูล {specialDay.name}</h1>
        </div>
        ...
        <div className="bg-white dark:bg-[#121212] rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800/80 p-8 md:p-10 ring-1 ring-zinc-900/5 dark:ring-white/5 relative overflow-hidden group">
            <form action={async (formData) => {
                "use server";
                await updateSpecialDay(id, formData);
                ...
                <div className="space-y-2">
                    ...
                    className="w-full rounded-2xl border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 px-5 py-3.5 text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all font-medium"
                    />
                </div>
                ...
                <div className="flex items-center gap-3 py-2">
                    ...
                    name="is_recurring"
                    id="is_recurring"
                    defaultChecked={specialDay.is_recurring}
