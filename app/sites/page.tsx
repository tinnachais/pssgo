        ...
        <div className="bg-white dark:bg-[#121212] rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800/80 overflow-hidden ring-1 ring-zinc-900/5 dark:ring-white/5">
            <div className="overflow-x-auto">
                ...
                <thead>
                    ...
                    <th className="px-6 py-5 text-left text-xs font-semibold text-zinc-500 uppercase tracking-widest w-[10%]">สถานะ</th>
                    ...
                    sites.map((site: any) => {
                        ...
                        <tr key={site.id} className="hover:bg-zinc-50 dark:hover:bg-[#1A1A1A] transition-all group">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white shadow-sm font-bold text-lg flex-shrink-0">
                                {site.name.charAt(0)}
                                </div>
                                <div>
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                    ...
                                    <span className="text-[10px] px-2 py-0.5 rounded font-medium bg-zinc-100 text-zinc-500 dark:bg-zinc-800/50 dark:text-zinc-400">ยังไม่มีโซน</span>
                                    )}
                                </div>
                                </div>
                            </div>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center gap-3">
                                    <div className="flex flex-col items-center px-2 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800/50 min-w-[60px]">
                                        <span className="text-xs text-blue-600 dark:text-blue-400 font-semibold mb-0.5">บ้าน</span>
                                    ...
                                    <div className="flex flex-col items-center px-2 py-1 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-100 dark:border-emerald-800/50 min-w-[60px]">
                                        <span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold mb-0.5">รถยนต์</span>
                                        <div className="text-sm font-black text-emerald-700 dark:text-emerald-300 flex items-baseline gap-0.5">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-purple-50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400 rounded-full text-xs font-semibold border border-purple-200 dark:border-purple-500/20">
                                    ...
                                    {site.provider_name || 'ไม่มีนิติบุคคล'}
                                </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                {(site.package_name || site.package_expires_at) ? (
                                    <div className="flex flex-col gap-1.5">
                                        <div className="font-bold text-sm text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5">
                                            ...
                                            {site.package_name || 'ไม่ระบุแพ็กเกจ'}
                                        </div>
                                        {site.package_expires_at && (
                                            <div className={`text-[11px] font-bold px-2 py-0.5 rounded inline-flex w-fit ${new Date(site.package_expires_at).setHours(0,0,0,0) < new Date().setHours(0,0,0,0) ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400'}`}>
                                                {new Date(site.package_expires_at).setHours(0,0,0,0) < new Date().setHours(0,0,0,0) ? 'หมดอายุเมื่อ: ' : 'หมดอายุ: '}
                                                {new Intl.DateTimeFormat('th-TH', { year: '2-digit', month: 'short', day: 'numeric' }).format(new Date(site.package_expires_at))}
                                            </div>
                                        )}
                                    </div>
                                ) : (
                                    <span className="text-xs text-zinc-400 font-medium">ไม่มีแพ็กเกจ</span>
                                )}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            <form action={async () => {
                            ...
                            <div className="flex justify-end gap-1">
                                <Link href={`/sites/${site.id}`} title="ดู/แก้ไขรายละเอียด" className="text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors p-2 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-500/10 inline-flex">
